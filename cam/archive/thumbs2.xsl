...
<xsl:template match="all" >
		...
		<xsl:for-each select="picture[@width $gt$ 268]" order-by="- date(file/date)">
		<a>
		<xsl:attribute name="href">JavaScript:popup('<xsl:value-of select="file/date"/>','<xsl:value-of select="meta/caption"/>','cam/archive/<xsl:value-of select="file/name"/>',<xsl:value-of select="@height"/>,<xsl:value-of select="@width"/>)</xsl:attribute>
		<img>
			<xsl:attribute name="src">http://www.onfocus.com/cam/archive/thumbs/<xsl:value-of select="file/name"/></xsl:attribute>
		...
		<tr>
			<td align="left" valign="center" bgcolor="#333333">
		<xsl:for-each select="picture[@width $lt$ 267]" order-by="- date(file/date)">
		<a>
		<xsl:attribute name="href">JavaScript:popup('<xsl:value-of select="file/date"/>','<xsl:value-of select="meta/caption"/>','cam/archive/<xsl:value-of select="file/name"/>',<xsl:value-of select="@height"/>,<xsl:value-of select="@width"/>)</xsl:attribute>
		<img>
			<xsl:attribute name="src">http://www.onfocus.com/cam/archive/thumbs/<xsl:value-of select="file/name"/></xsl:attribute>
